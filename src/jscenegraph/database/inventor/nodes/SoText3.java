...
public void convertToUCS(int nodeid, SoMFString strings) {
    ...
    CharBuffer     getUCSString(int line)
...
//Use: private
    ...
    float total = 0.0f;
    CharBuffer chars = getUCSString(line);
    
    for (int i = 0; i < getNumUCSChars(line); i++) {
        SoFontOutline outline = getOutline(chars.charAt(i));
...
////////////////////////////////////////////////////////////////////////
{
    ...
    final Object[] value = new Object[1];
    for (int i = 0; i < getNumUCSChars(line); i++) {
        int key = Character.reverseBytes(ustr.charAt(i));//(ustr[2*i]<<8)|ustr[2*i+1]; // java port
        if (sideDict.find(key, value)) {
            gl2.glCallList(sideList.getFirstIndex()+key);
        }
        else {
            gl2.glBegin(GL2.GL_QUADS);
            generateSideChar(str.charAt(i), callbackFunc);
            gl2.glEnd();

            final SbVec2f t = getOutline(str.charAt(i)).getCharAdvance();
...
////////////////////////////////////////////////////////////////////////
{
    ...
    final Object[] value = new Object[1];
    for (int i = 0; i < getNumUCSChars(line); i++) {
        int key = Character.reverseBytes(ustr.charAt(i));//ustr[2*i]<<8 | ustr[2*i+1];  
        if (frontDict.find(key, value)) {
            gl2.glCallList(frontList.getFirstIndex()+key);
        }
        else {
            generateFrontChar(gl2, str.charAt(i), tobj);
            SbVec2f t = getOutline(str.charAt(i)).getCharAdvance();
            gl2.glTranslatef(t.getValue()[0], t.getValue()[1], 0.0f);
        }
    }
}
...
////////////////////////////////////////////////////////////////////////
{
    ...
    CharBuffer chars = myFont.getUCSString(line);
    ...
    for (int i = 0; i < myFont.getNumUCSChars(line); i++) {
        ...
        if (!myFont.hasFrontDisplayList(gl2, chars.charAt(i), tobj)) {
            useCallLists = false;
            break;
        }
    }
...
////////////////////////////////////////////////////////////////////////
{
    ...
    for (int i = 0; i < myFont.getNumUCSChars(line); i++) {
        // See if the font cache already has (or can build) a display
        // list for this character:
        if (!myFont.hasSideDisplayList(gl2, chars.get(i),  new SideCB() {
...
////////////////////////////////////////////////////////////////////////
{
    ...
    for (line = 0; line < string.getNum(); line++) {
        ...
        for (character = 0; character < myFont.getNumUCSChars(line); 
                character++) {
            myFont.getCharBBox(chars.get(character), charBBox);
            if (!charBBox.isEmpty()) {
            ...
            // And advance...
            charPosition.operator_add_equal( myFont.getCharOffset(chars.get(character)));
        }
    }
}
...
////////////////////////////////////////////////////////////////////////
{
    ...
    for (int i = 0; i < myFont.getNumUCSChars(line); i++) {
        ...
        myFont.generateSideChar(chars.get(i),new SideCB() {
        ...
        SbVec2f p = myFont.getCharOffset(chars.get(i));
...
////////////////////////////////////////////////////////////////////////
{
    ...
    for (int i = 0; i < myFont.getNumUCSChars(line); i++) {
        ...
        myFont.generateFrontChar(gl2, chars.get(i), tobjgenerateFront);
